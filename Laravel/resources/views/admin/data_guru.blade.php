@extends('admin.layout.main')

@section('title', 'Data Guru - EDUTRACK')

@section('content')
<div class="content">
    <h2 class="page-title">Data Guru</h2>

    <div class="card p-4 shadow-sm">

        <div class="add-btn-container mb-3" style="display:flex; justify-content:flex-end; width:100%;">
            <a href="{{ route('admin.tambah_guru') }}" class="btn btn-success">
                <i class="fa fa-plus"></i> Tambah Guru
            </a>
        </div>

        <table class="biodata-table table table-bordered table-striped" style="width:100%;">
            <thead style="background-color:#f1f5f9;">
                <tr>
                    <th style="padding:10px;">No</th>
                    <th style="padding:10px;">Nama Guru</th>
                    <th style="padding:10px;">Tanggal Lahir</th>
                    <th style="padding:10px;">Alamat</th>
                    <th style="padding:10px;">Mata Pelajaran</th>
                    <th style="padding:10px;">NIP</th>
                    <th style="padding:10px;">Kelas</th>
                    <th style="padding:10px;">Jenis Kelamin</th>
                    <th style="padding:10px;">Aksi</th>
                </tr>
            </thead>
            <tbody>
                @foreach($guruList as $index => $guru)
                    <tr>
                        <td style="padding:10px;">{{ $index + 1 }}</td>
                        <td style="padding:10px;">{{ $guru->name }}</td>
                        <td style="padding:10px;">{{ \Carbon\Carbon::parse($guru->date_of_birth)->format('d-m-Y') }}</td>
                        <td style="padding:10px;">{{ $guru->address }}</td>
                        <td style="padding:10px;">{{ $guru->mapel ? $guru->mapel->name : '-' }}</td>
                        <td style="padding:10px;">{{ $guru->nip }}</td>
                        <td style="padding:10px;">
                            {{ $guru->kelas ? $guru->kelas->class . ' ' . $guru->kelas->subclass : '-' }}
                        </td>
                        <td style="padding:10px;">
                            {{ $guru->gender == 'male' ? 'Laki-laki' : 'Perempuan' }}
                        </td>
                        <td style="padding:20px;">
                            <div class="d-flex justify-content-center gap-2">
                                {{-- Tombol Edit --}}
                                <a href="{{ route('admin.edit_guru', $guru->id) }}"
                                   class="btn btn-primary btn-sm d-flex align-items-center">
                                    <i class="fa fa-pen"></i>
                                    <span class="ms-1">Edit</span>
                                </a>

                                {{-- Tombol Hapus dengan SweetAlert --}}
                                <form id="delete-form-{{ $guru->id }}"
                                      action="{{ route('admin.hapus_guru', $guru->id) }}"
                                      method="POST"
                                      style="display:inline;">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button"
                                            class="btn btn-danger btn-sm d-flex align-items-center"
                                            onclick="confirmDelete({{ $guru->id }})">
                                        <i class="fa fa-trash"></i>
                                        <span class="ms-1">Hapus</span>
                                    </button>
                                </form>
                            </div>
                        </td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    </div>
</div>

@include('admin.components.sweetalert')
@endsection
